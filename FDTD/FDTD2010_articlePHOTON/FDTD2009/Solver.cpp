 ...
 //   D =sin(M_PI*S2D/Nx)*sin(M_PI*S2D/Nx);
	...
	for(int i = 0; i < dataTE->gridX; i++)
		for(int j = 0; j < dataTE->gridY; j++)
		{
			...
			if( (j>= LBslab) && (j<=RBslab) && (i>=25) && (i<=dataTE->gridX-25))
			{
				omegaPE[i][j] = sqrt(2.0)*OMEGA;
				omegaPM[i][j] = sqrt(2.0)*OMEGA;
				gammaE[i][j] = 0;//0.05*OMEGA;
				gammaM[i][j] = 0;//0.05*OMEGA;
			}


			if(j==35)
			{
				gammaE[i][j] = 0.5*OMEGA;
				gammaM[i][j] = 0.5*OMEGA;
			}
			Gz[i][j] = 0.0;
			Fz[i][j] = 0.0;
			Gzeven[i][j] = 0.0;
...
void CSolver::DeleteSimpleFDTD_TE_UPML (void)
{
		for(int i = 0; i < dataTE->gridX; i++)
		{
			delete []Fz[i];
...
void CSolver::SimpleFDTDSolver_TE_UPML(int timestep)
{
	...
	double A = 1;
	double B = 1;
	double Kx = 2.0*M_PI/LAMBDA;
	double T0 = 4800;
	double m = 10;
	
	for(int i = center-sourceWidth+1; i < center+sourceWidth; i++)
	{
		A = Kx*exp(-((i-center)/(((double)sourceWidth/50.0)*sqrt((double)sourceWidth)))*((i-center)/(((double)sourceWidth/50.0)*sqrt((double)sourceWidth))));
        B = exp(-(double)(timestep-t0)*(timestep-t0)/((double)(t0*t0)/10.0));
		dataTE->Hz[i][25] = A*sin(OMEGA*dt*timestep) + dataTE->Hz[i][25];
	}
	
	
	for(int j = 25; j <=35; j++)
	{
		dataTE->Hz[center-sourceWidth][j] = 0;
		dataTE->Hz[center+sourceWidth][j] = 0;
	}

	for(int i = 20; i <=center-sourceWidth; i++)
	{
		dataTE->Hz[i][35] = 0;
		dataTE->Hz[i][35] = 0;
	}

	for(int i = center+sourceWidth; i <=dataTE->gridX-20; i++)
	{
		dataTE->Hz[i][35] = 0;
		dataTE->Hz[i][35] = 0;
	}

	for(int i = 20; i <=dataTE->gridX-20; i++)
	{
		dataTE->Hz[i][395] = 0;
		dataTE->Hz[i][395] = 0;
	}
...
void CSolver::TimeStepLoopTE(int N, string path)
{
	for(int t = 0; t < N; t++)
	{
	   ...
	   // dataTE->InitNetCDF(t, dx);
		...
		{
			if(t%5 == 0) 
				dataTE->WriteToBinHz(t/5,pmlLR,pmlUD,pmlLR,pmlUD, path);
			if(t%5 == 0) 
				dataTE->WriteToBinEx(t/5,pmlLR,pmlUD,pmlLR,pmlUD, path);
			if(t%5 == 0) 
				dataTE->WriteToBinEy(t/5,pmlLR,pmlUD,pmlLR,pmlUD, path);
		}
