...
CSolver::CSolver(const char* par, int SizeX, int SizeY, int SizeZ)
{
	...
	if (par == "TE")
	{
		...
		CaF = new double*[dataTE->gridX];
		CbF = new double*[dataTE->gridX];
		CcF = new double*[dataTE->gridX];
		CdF = new double*[dataTE->gridX];
		CeF = new double*[dataTE->gridX];
		CfF = new double*[dataTE->gridX];
		...
		for(int i = 0; i < dataTE->gridX; i++)
		{
			...
			CaF[i] = new double[dataTE->gridY];
			CbF[i] = new double[dataTE->gridY];
			CcF[i] = new double[dataTE->gridY];
			CdF[i] = new double[dataTE->gridY];
			CeF[i] = new double[dataTE->gridY];
			CfF[i] = new double[dataTE->gridY];
...
void CSolver::Init1DSimpleFDTD(void)
{
	...
	{cout << "Warning::Computed dt is greater than Nyquist: " << dt << endl; system("PAUSE");}
		...
		for(int i = data1D->gridX/2; i < (data1D->gridX/2)+60; i++)
	{
		Eps1D[i] = 7.0;
		Mju1D[i] = 7.0;
	}
	...
	for(int i = 0; i < Nx; i++)
	{
		Ca1D[i] = (1.0 - Sig1D[i] * dt / (2.0 * Eps1D[i] * EPS_Z)) / (1.0 + Sig1D[i] * dt / (2.0 * Eps1D[i] * EPS_Z));
		Cb1D[i] = ( dt / (2.0 * Eps1D[i] * EPS_Z * dx)) / (1.0 + Sig1D[i] * dt / (2.0 * Eps1D[i] * EPS_Z));

		Da1D[i] = (1.0 - SigS1D[i] * dt / (2.0 * Mju1D[i] * MU_Z)) / (1.0 + SigS1D[i] * dt / (2.0 * Mju1D[i] * MU_Z));
		Db1D[i] = ( dt / (2.0 * Mju1D[i] * MU_Z * dx)) / (1.0 + SigS1D[i] * dt / (2.0 * Mju1D[i] * MU_Z));
	}
...
void CSolver::Simple1DFDTDSolver(int timestep)
{
	...
	{
		data1D->Ez[i] = Ca1D[i] * data1D->Ez[i] + Cb1D[i] * (data1D->Hy[i] - data1D->Hy[i-1]);
	}

	double xcord = (data1D->gridX/2-200)*dx;
	data1D->Ez[data1D->gridX/2-200] = data1D->Ez[data1D->gridX / 2 - 200] - 
		(cos(OMEGA * dt * (timestep + 0.5) - xcord * kvec) - cos(OMEGA * dt * (timestep - 0.5) - xcord * kvec)) +
		Cb1D[data1D->gridX / 2 - 200] * (sin(OMEGA * dt * timestep - (xcord + 0.5 * dx) * kvec) - 
		sin(OMEGA * dt * timestep - (xcord - 0.5 * dx) * kvec));

	for(int i = 1; i < data1D->gridX-1; i++)
	{
		data1D->Hy[i] = Da1D[i] * data1D->Hy[i] + Db1D[i] * (data1D->Ez[i+1] - data1D->Ez[i]);
	}
	data1D->Hy[data1D->gridX/2-200] = data1D->Hy[data1D->gridX / 2 - 200] + 
		(sin(OMEGA * dt * (timestep + 1) - xcord * kvec) - sin(OMEGA * dt * (timestep) - xcord * kvec)) +
		Db1D[data1D->gridX / 2 - 200] * (cos(OMEGA * dt * (timestep + 0.5) - xcord * kvec) - 
		cos(OMEGA * dt * (timestep + 0.5) - (xcord - dx) * kvec));
	//data1D->Hy[data1D->gridX/2-200] = data1D->Hy[data1D->gridX/2] + cos(OMEGA*dt*timestep);
}
 ...
 //   D =sin(M_PI*S2D/Nx)*sin(M_PI*S2D/Nx);
	...
	for(int i = 0; i < dataTM->gridX; i++)
		for(int j = 0; j < dataTM->gridY; j++)
		{
			...
			//if((i== LBslab) || (i==RBslab))
			//	CdF[i][j] = ((1.0/1.0) - (dt*gammaE[i][j]/(2.0)) + dt*dt*omegaPE[i][j]*omegaPE[i][j]/8.0);
			//else
				CdF[i][j] = ((1.0/1.0) - (dt*gammaE[i][j]/(2.0)) + dt*dt*omegaPE[i][j]*omegaPE[i][j]/4.0);
			CeF[i][j] = (1.0/( plug)) - (dt*gammaE[i][j]/(2.0*plug));
			//if((i== LBslab) || (i==RBslab))
			//	CfF[i][j] = ((1.0) + (dt*gammaE[i][j]/(2.0)) + dt*dt*omegaPE[i][j]*omegaPE[i][j]/8.0);
			//else
			...
			CbT[i][j] = 2.0/( plug);
			if((i== LBslab) || (i==RBslab))
				CcT[i][j] = ((2.0/1.0) - dt*dt*omegaPM[i][j]*omegaPM[i][j]/4.0);
			else
				CcT[i][j] = ((2.0/1.0) - dt*dt*omegaPM[i][j]*omegaPM[i][j]/2.0);
			if((i== LBslab) || (i==RBslab))
				CdT[i][j] = ((1.0/1.0) - (dt*gammaM[i][j]/(2.0)) + dt*dt*omegaPM[i][j]*omegaPM[i][j]/8.0);
			else
				CdT[i][j] = ((1.0/1.0) - (dt*gammaM[i][j]/(2.0)) + dt*dt*omegaPM[i][j]*omegaPM[i][j]/4.0);
			CeT[i][j] = (1.0/( plug)) - (dt*gammaM[i][j]/(2.0*plug));
			if((i== LBslab) || (i==RBslab))
				CfT[i][j] = ((1.0) + (dt*gammaM[i][j]/(2.0)) + dt*dt*omegaPM[i][j]*omegaPM[i][j]/8.0);
			else
 ...
 //   D =sin(M_PI*S2D/Nx)*sin(M_PI*S2D/Nx);
	...
	for(int i = 0; i < dataTE->gridX; i++)
		for(int j = 0; j < dataTE->gridY; j++)
		{
			...
			if( (i>= LBslab) && (i<=RBslab) && (j>20) && (j<dataTE->gridY-20))
			{
				omegaPE[i][j] = sqrt(2.0)*OMEGA;
				omegaPM[i][j] = sqrt(2.0)*OMEGA;
				gammaE[i][j] = 0;//0.0005*OMEGA;
	...
	for(int i = 0; i < dataTE->gridX; i++)
		for(int j = 0; j < dataTE->gridY; j++)
		{
			...
			CbF[i][j] = 2.0/( plug);
			CcF[i][j] = ((2.0/1.0) - dt*dt*omegaPE[i][j]*omegaPE[i][j]/2.0);
			//if((i== LBslab) || (i==RBslab)||(i== LBslab-1) || (i==RBslab-1)||(i== LBslab+1) || (i==RBslab+1))
			//	CdF[i][j] = ((1.0/1.0) - (dt*gammaE[i][j]/(2.0)) + dt*dt*omegaPE[i][j]*omegaPE[i][j]/8.0);
			//else
				CdF[i][j] = ((1.0/1.0) - (dt*gammaE[i][j]/(2.0)) + dt*dt*omegaPE[i][j]*omegaPE[i][j]/4.0);
			CeF[i][j] = (1.0/( plug)) - (dt*gammaE[i][j]/(2.0*plug));
...
void CSolver::SimpleFDTDSolver_TE_UPML(int timestep)
{
	...
	for(int i = 1; i < (dataTE->gridX - 1); i++)
		for(int j = 1; j < (dataTE->gridY - 1); j++)
		{
			...
			Tx[i][j] = (CaF[i][j]*Bx[i][j] - CbF[i][j]*Bx_pr + CcF[i][j]*Tx_pr -
				CdF[i][j]*Txtmp + CeF[i][j]*Bxtmp)/CfF[i][j];
...
int center = dataTE->gridY/2;
	...
	for(int j = center-sourceWidth+1; j < center+sourceWidth; j++)
	{
		A = 25.0*exp(-((j-center)/(((double)sourceWidth/50.0)*sqrt((double)sourceWidth)))*((j-center)/(((double)sourceWidth/50.0)*sqrt((double)sourceWidth))));
        B = exp(-(double)(timestep-t0)*(timestep-t0)/((double)(t0*t0)/10.0));
		dataTE->Hz[25][j] = A*sin(OMEGA*dt*timestep) + 0.1*dataTE->Hz[25][j];
	}
...
void CSolver::TimeStepLoop1D(int  N)
{
	for (int t = 0; t <=N; t++)
	{
		...
		cout << " ...done." << endl;
		data1D->WriteToBinEz(t, 0, 0);
		data1D->WriteToBinHy(t, 0, 0);
	}
}
...
void CSolver::TimeStepLoopTE(int N)
{
	for(int t = 0; t < N; t++)
	{
	   ...
	   // dataTE->InitNetCDF(t, dx);
		...
		if(t>=3000 )
		{
			if(t%5 == 0) dataTE->WriteToBinHz(t/5,20,20,20,20);
			if(t%5 == 0) dataTE->WriteToBinEx(t/5,20,20,20,20);
			if(t%5 == 0) dataTE->WriteToBinEy(t/5,20,20,20,20);
		}
