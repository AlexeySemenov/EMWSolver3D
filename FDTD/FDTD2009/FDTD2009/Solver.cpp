 ...
 //   D =sin(M_PI*S2D/Nx)*sin(M_PI*S2D/Nx);
	...
	for(int i = 0; i < dataTM->gridX; i++)
		for(int j = 0; j < dataTM->gridY; j++)
		{
			...
			if((i-Nx/2)*(i-Nx/2)+(j-Ny/2)*(j-Ny/2) <= (int)(bcyl*bcyl))
			if((i-Nx/2)*(i-Nx/2)+(j-Ny/2)*(j-Ny/2) > (int)(acyl*acyl))
			{
				rcyl = sqrt((double)((i-Nx/2)*(i-Nx/2)+(j-Ny/2)*(j-Ny/2)));
				Eps[i][j] = ((rcyl-acyl)/rcyl)*((rcyl-acyl)/rcyl);
				Mju[i][j] = ((rcyl-acyl)/rcyl)*((rcyl-acyl)/rcyl);
			}
	...
	for(int i = 0; i < dataTM->gridX; i++)
		for(int j = 0; j < dataTM->gridY; j++)
		{
			...
			CbGz[i][j] = 2*EPS_Z/(2*EPS_Z * K_x(i) + Sig_x(i,true)*dt);
			...
			DbHx[i][j] = (2*EPS_Z*K_x(i) + Sig_x(i,true)*dt)/
				((2*EPS_Z*K_z(z) + Sig_z(z)*dt)*MU_Z*Mju[i][j]);
			...
			DbHy[i][j] = (2*EPS_Z*K_y(j) + Sig_y(j,true)*dt)/
				((2*EPS_Z*K_x(i) + Sig_x(i,false)*dt)*MU_Z*Mju[i][j]);
...
void CSolver::SimpleFDTDSolver_TM_UPML(int timestep)
{
	...
	for(int i = 1; i < (dataTM->gridX - 1); i++)
		for(int j = 1; j < (dataTM->gridY - 1); j++)
		{
			...
			Fz_pr = Fz[i][j];
			Gz_pr = Gz[i][j];
			Fz[i][j] =  CaFz[i][j]*Fz[i][j] + (CbFz[i][j]/dx)*((dataTM->Hy[i][j] - dataTM->Hy[i-1][j]) + 
					(dataTM->Hx[i][j-1] - dataTM->Hx[i][j]));
			Gz[i][j] = CaGz[i][j]*Gz[i][j] + CbGz[i][j]*(Fz[i][j]-Fz_pr);
	...
	//dataTM->Ez[50][500] = 10*sin(OMEGA*dt*timestep);
	//Update H - Field
	for(int i = 1; i < (dataTM->gridX - 1); i++)
		for(int j = 1; j < (dataTM->gridY - 1); j++)
		{
			...
			dataTM->Hx[i][j] = DaHx[i][j]*dataTM->Hx[i][j]+ DbHx[i][j]*Bx[i][j]-
				DcHx[i][j]*Bx_pr;
			...
			dataTM->Hy[i][j] = DaHy[i][j]*dataTM->Hy[i][j]+ DbHy[i][j]*By[i][j]-
				DcHy[i][j]*By_pr;
