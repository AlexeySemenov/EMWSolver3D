 ...
 //   D =sin(M_PI*S2D/Nx)*sin(M_PI*S2D/Nx);
	...
	for(int i = 0; i < dataTM->gridX; i++)
		for(int j = 0; j < dataTM->gridY; j++)
		{
			...
			if( (i>= 300) && (i<=335) && (j>20) && (j<dataTM->gridY-20))
			{
				omegaPE[i][j] = sqrt(2.0)*OMEGA;
				omegaPM[i][j] = sqrt(2.0)*OMEGA;
				gammaE[i][j] = 0.0005*OMEGA;
				gammaM[i][j] = 0.0005*OMEGA;
			}
...
void CSolver::SimpleFDTDSolver_TM_UPML(int timestep)
{
	...
	for(int i = 0; i < (2*(dataTM->gridX) - 1); i++)
		...
		dataTM->Ez[300][dataTM->gridY/2-17] = sin(OMEGA*dt*timestep);//exp(-((timestep-300)*dt/100.0))*sin(OMEGA*dt*(timestep-300));
		dataTM->Ez[300][dataTM->gridY/2+17] = sin(OMEGA*dt*timestep);//exp(-((timestep-300)*dt/100.0))*sin(OMEGA*dt*(timestep-300));
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	double todel;
	//if(timestep>9)
...
void CSolver::TimeStepLoop(int N)
{
	for(int t = 0; t < N; t++)
	{
	   ...
	   // dataTE->InitNetCDF(t, dx);
		...
		if(t>= 2000)
		if(t%5 == 0) dataTM->WriteToNetCDF(t/5,20,20,20,20);
